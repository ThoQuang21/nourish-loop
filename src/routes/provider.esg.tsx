...
import { esgWeekly } from "@/lib/mock-data";
...
function ESGPage() {
  const stats = [
    { label: "Tổng thực phẩm chia sẻ", value: "1,840 kg", delta: "+12%" },
    { label: "Giao dịch thành công", value: "124", delta: "+8%" },
    { label: "Ước tính CO₂ giảm", value: "5.2 tấn", delta: "+15%" },
    { label: "Người hưởng lợi", value: "~ 3,200", delta: "+22%" },
  ];
  return (
    <div className="p-6 lg:p-10 max-w-6xl mx-auto space-y-8">
      <header>
        ...
        <p className="text-muted-foreground mt-1">
          Dữ liệu 30 ngày qua – chia sẻ báo cáo với đối tác và nhà đầu tư.
        </p>
      </header>
        ...
        {stats.map((s) => (
          <div key={s.label} className="bg-card border border-border rounded-2xl p-5 shadow-card">
            ...
            <div className="flex items-baseline justify-between mt-2">
              ...
              <span className="text-xs font-semibold text-primary bg-primary-soft/40 px-2 py-0.5 rounded-full">
                {s.delta}
              </span>
            </div>
          </div>
